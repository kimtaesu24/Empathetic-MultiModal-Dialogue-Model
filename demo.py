...
def audio2feature(SPEECH_FILE, desired_sampling_rate=16000):
    ...
    resampled_audio = librosa.resample(audio, sr, desired_sampling_rate)
    # Save the resampled audio to a new file
    sf.write(SPEECH_FILE, resampled_audio, desired_sampling_rate)
...
def preprocess_video(input_video, max_length):
    ...
    inputs = [image_list, audio_path, tokens, transcript, waveform, landmark]
    '''
...
def main(param, hyper_param, input_video, checkpoint):
    ...
    weigth = checkpoint + \
        "_epochs{'epochs': 200, 'act': 'relu', 'batch_size': 32, 'learning_rate': 5e-05, 'max_length': 60, 'alpha': 2, 'dropout': 0.2, 'decay_rate': 0.98}.pt"
    ...
    weight_path = path + \
        f"{landmark_append}/{give_weight}/{modal_fusion}/{forced_align}/{trans_encoder}/{multi_task}/" + weigth
    model = MyArch(param=param, hyper_param=hyper_param)

    model.load_state_dict(torch.load(
        weight_path, map_location=param['device']))
    model.eval()
    ...
    inputs.append(input_historys_tokens)
    '''
